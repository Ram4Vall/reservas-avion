...
function cambiarIdioma(arrayIdiomas,arrayPlaceHolder){
    for(var i=0;i<arrayIdiomas.length;i++){
        $("#span"+(i+1)).html(arrayIdiomas[i]);
    }
    var arrayElWPH = $("input[placeholder]");
    for(var i=0;i<arrayPlaceHolder.length;i++){
        $(arrayElWPH[i]).attr("placeholder",arrayPlaceHolder[i]);
    }
}
